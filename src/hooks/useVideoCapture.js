import { useState, useRef, useCallback, useEffect, useRef } from "react";
...
export const useVideoCapture = () => {
            ...
            if (videoRef.current) {
                ...
                videoRef.current.onloadedmetadata = () => {
                    videoRef.current.play()
                        ...
                        .catch(err => setError('비디오 재생에 실패했습니다: ' + err.message));
                };
    ...
    // 카메라 재시작
    const restartCamera = useCallback(async (options) => {
        stopCamera();
    ...
    // 해상도 변경
    const changeResolution = useCallback(async (resolution) => {
